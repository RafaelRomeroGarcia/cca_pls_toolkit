function [wX, wY] = rcca(data, featid, param, ncomp)
% rcca
%
...
%   add number of RCCA components as input
...
[wY, lambda] = eigs(diag(1./sqrt(BYY)) * RXY' * diag(1./BXX) * RXY * diag(1./sqrt(BYY)), ncomp);
lambda = real(sqrt(lambda));

% Compute weights
wY = diag(1./sqrt(BYY)) * wY;
wX = diag(1./BXX) * RXY * wY * diag(1./diag(lambda));