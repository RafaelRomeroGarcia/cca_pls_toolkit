...
%   fix output NaN in correct shape if all (or all but one) splits have NaN weights (e.g. due to SPLS not converging)
%   remove abs from correlation_pairwise as it's not needed due to align_weight.m
...
for el=1:numel(J)
    switch type
        case 'correlation'
            S(el) = correlation_pairwise(W(:,I(el)), W(:,J(el)), subtype);
