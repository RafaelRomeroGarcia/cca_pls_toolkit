...
%   fix deflation if number of associative effects is 1
%   add cfg fields for boostrapping analysis
%   add cfg.env.save.svd for optional saving of SVD files
%   add cfg for table heading in results summary table
%   fix cfg.defl.name if exvar used in metric
%   turn off some MATLAB warnings
%   set cfg.machine.svd.varx/vary to 0.99 by default in all CCA like analysis
%   fix error message for cfg.stat.crit if not correl
...
% Number of bootstrap samples and permutations in statistical inference
def.stat.nboot = 0;
def.stat.nperm = 1000;
...
if ~isfield(cfg.frwork, 'nlevel') || cfg.frwork.nlevel > 1 || ...
        any(contains(cfg.machine.metric, 'exvar'))
...
def.env.save.compression = 1; % we save compressed files
def.env.save.svd = 1; % save SVD results
...
% Define modality for aligning weights based on Procrustes analysis
if size(data.Y, 'Y', 2) == 1
    def.machine.alignw = 'wX';
else
    def.machine.alignw = 'wY';
end

% Maximum number of associative effects
if isfield(cfg, 'defl') && isfield(cfg.defl, 'name') && strcmp(cfg.defl.name, 'pls-regression')
...
if ismember(cfg.machine.name, {'pls' 'spls'})
    ...
    def.machine.svd.tol = 1e-10;
     ...
     % Set cutoff to explained variance to avoid overfitting
    if contains(cfg.machine.name, 'cca')
        def.machine.svd.varx = 0.99;
        def.machine.svd.vary = 0.99;
    end
end
...
% Results to be saved in summary table
if strcmp(cfg.frwork.name, 'holdout')
    def.env.save.tableHeading = {'split'};

elseif contains(cfg.frwork.name, 'permutation')
    def.env.save.tableHeading = {'set'};
end
def.env.save.tableHeading = [def.env.save.tableHeading {cfg.stat.crit 'pval'}];
for j=1:numel(cfg.machine.param.name)
    m = cfg.machine.param.name{j}(end); % shorthand variable
    if strcmp(cfg.machine.param.name{j}, ['L1' m])
        def.env.save.tableHeading = [def.env.save.tableHeading {['nfeat' m]}];

    elseif strcmp(cfg.machine.param.name{j}, ['PCA' m])
        def.env.save.tableHeading = [def.env.save.tableHeading {['npca' m]}];

    else
        def.env.save.tableHeading = [def.env.save.tableHeading {lower(cfg.machine.param.name{j})}];
    end
end
if isfield(cfg, 'defl') && ~isempty(strfind(cfg.defl.crit, 'sim'))
    def.env.save.tableHeading = [def.env.save.tableHeading {'dist'}];
end
...
if cfg.stat.nperm > 0
    if strcmp(cfg.machine.name, 'pls') || (strcmp(cfg.machine.name, 'rcca') ...
            && all(cfg.machine.param.L2x == 1) && all(cfg.machine.param.L2y == 1))
        if ~ismember(cfg.stat.crit, {'correl' 'covar'})
