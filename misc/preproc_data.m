function data = preproc_data(res, data, mod, fname)
% preproc_data
%
% # Syntax
%   data = preproc_data(res, data, mod, fname)
%
...
%   refactor zscoring
%   use fname input as relative path
...
for i=1:numel(preproc)
    if strcmp(preproc{i}, 'zscore')
        ...
        if strncmp(fname, 'tr_', 3) && ~exist_file(cfg, fname(4:end))
            [data.(mod), mu] = mean_center_features(data.(mod));
            [data.(mod), sigma] = norm_features(data.(mod), [], 'std');
        else
            [mu, sigma] = loadmat(cfg, fullfile(cfg.dir.load, 'preproc', fname(4:end)), 'mu', 'sigma');
    ...
    elseif strcmp(preproc{i}, 'deconf')
        if strcmp(cfg.data.(mod).deconf, 'standard')
            ...
            if strncmp(fname, 'tr_', 3) && ~exist_file(cfg, fname(4:end))
                ...
                beta = loadmat(cfg, fullfile(cfg.dir.load, 'preproc', fname(4:end)), 'beta');
            end
...
% Save file
fname = fullfile(cfg.dir.load, 'preproc', fname(4:end));
