function plot_weight_brain_module(res, T, wfname)
% plot_weight_brain_module
%
% # Syntax
%   plot_weight_brain_module(res, T, wfname)
%
...
T = cat(2, T, readlabel(labelfname, 'connectivity', T.Properties.VariableNames, {'Region'}));

% Load mask
maskfname = select_file(res, fullfile(res.dir.project, 'data'), ...
    'Select mask file...', 'mat', res.conn.file.mask);
load(maskfname);
if sum(mask(:)) ~= numel(T.weight)
...
conn_weight(mask==1) = T.weight; % fill up using linear indexing
