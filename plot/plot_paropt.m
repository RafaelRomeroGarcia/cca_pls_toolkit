...
%   fix plot_2D to use split-specific parameter
...
for i=1:numel(metrics)
    ...
    elseif strcmp(cfg.machine.param.type, 'matched')
        S.(metrics{i}) = reshape(S.(metrics{i}), numel(get_hyperparam(res, 'default')), res.frwork.split.nall);
    end
end
...
if numel(p) == 1 || strcmp(cfg.machine.param.type, 'matched')
    % Line plot as a function of 1 hyperparameter
    for i=1:numel(metrics)
        subplot(1, numel(metrics), i);
        plot_2D(res, S.(metrics{i})(:,split), param(split), metrics{i});
    end
...
function plot_2D(res, data, param, ylab)
...
% Plot data
if any(ismember(fields{1}, {'L2x' 'L2y'}))
    plot(1-cfg.machine.param.(fields{1}), data);
    hold on;
    plot(gca, 1-repmat(param.(fields{1}), 1, 2), get(gca, 'YLim'), 'r', 'LineWidth', 2);
    if numel(fields) == 1
        xlabel(['1 - ' fields{1}]);
    else
        xlabel(['1 - ' strjoin(fields, ' | ')]);
    end
    
else
    plot(cfg.machine.param.(fields{1}), data);
    xlabel(fields{1});
end
