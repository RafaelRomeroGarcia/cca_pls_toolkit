function plot_weight_behav_horz(res, T, wfname)
% plot_weight_behav_horz
%
% # Syntax
%   plot_weight_behav_horz(res, T, wfname)
%
...
%   add legend only if > 1 categories exist
...
if isnumeric(T.Category)
    T.Category = sprintfc('%d', T.Category);
end
...
h = zeros(1, nweights);
for i=1:nweights
    h(i) = barh(x(i), T.weight(i), 'FaceColor', T.Color(i,:));
end

% Plot error bar
if sum(sum(isnan(T.errorbar))) ~= size(T, 1) * 2
    id = T.weight ~= 0; % remove 0 weights
    er = errorbar(T.weight(id), x(id), T.errorbar(id, 1), T.errorbar(id, 2), 'horizontal');
