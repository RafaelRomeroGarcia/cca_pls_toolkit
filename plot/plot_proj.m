...
%   fix filename for an edge case
%   use transform_data.m for SVD inverse feature transformation
%   refactor code a bit
...
for i=1:nlevels
    for j=1:nmods
        ...
        if ismember(cfg.machine.name, {'pls' 'spls'})
            ...
            if ismember(sidvar, {'otrid' 'itrid' 'oext'})
                [trdata, trid] = load_data(res, {['R' mod{i,j}]}, ...
                    sidvar, squeeze(split(i,j,:)), param);
            else
        ...
        if ismember(cfg.machine.name, {'pls' 'spls'})
            ...
            featid = get_featid(trdata, param, mod{i,j});
            
            % RCCA inverse feature transformation
            w = transform_data(cfg, 'svd-inverse-transform', w, mod{i,j}, ...
                'V', trdata.(['V' mod{i,j}])(:,featid));
...
elseif numel(colour) > 1 % groups based on group.mat
...
if ~strcmp(colour{1}, 'none') && numel(colour) == 1
